...
class CommonSensePredicateStatisticsSplitArgs():
    ...
    def __init__(self,
                 ...
                 base_trace_path: typing.Optional[typing.Union[str, pathlib.Path]] = None,
        ...
        trace_names_hash = stable_hash_list(trace_names)[:trace_hash_n_characters]
        ...
        if os.path.exists(stats_filename) and not overwrite:
            ...
            if base_trace_path is None:
                raise ValueError("Must provide a valid path to traces if cache does not exist")
            
            print(f"No cache file found at {stats_filename}. Building from scratch...")

            trace_paths = [os.path.join(get_project_dir(), base_trace_path, f"{trace_name}.json") for trace_name in trace_names]
...
if __name__ == '__main__':
    ...
    stats = CommonSensePredicateStatisticsSplitArgs(cache_dir=DEFAULT_CACHE_DIR,
                                                    trace_names=CURRENT_TEST_TRACE_LIST,
