...
(same_type <arg1> <arg2>) [5 references] ; Are these two objects of the same type? Or if one is a direct reference to a type, is this object of that type?
...
(*\color{gray} agent [N/A references] ; The agent*)
(*\color{gray} building [N/A references] ; Not a real game object, but rather, a way to refer to structures the agent builds*)
---------- (* \textbf{Blocks} *) ----------
block [2 references] ; Parent type of all block types:
bridge_block [5 references] 
cube_block [12 references] 
blue_cube_block [7 references] 
(*\color{gray} tan_cube_block [N/A references] *)
yellow_cube_block [7 references] 
flat_block [2 references] 
pyramid_block [9 references] 
blue_pyramid_block [2 references] 
red_pyramid_block [1 reference] 
triangle_block [3 references] 
yellow_pyramid_block [1 reference] 
cylindrical_block [8 references] 
tall_cylindrical_block [5 references] 
---------- (* \textbf{Balls} *) ----------
ball [2 references] ; Parent type of all ball types:
beachball [17 references] 
basketball [18 references] 
dodgeball [49 references] 
blue_dodgeball [6 references] 
(*\color{gray} red_dodgeball [N/A references] *)
pink_dodgeball [6 references] 
golfball [23 references] 
(*\color{gray} green_golfball [N/A references] *)
---------- (* \textbf{Colors} *) ----------
(*\color{gray} color [N/A references] ; Likewise, not a real game object, mostly used to refer to the color of the rug under an object*)
blue [1 reference] 
brown [1 reference] 
green [5 references] 
pink [12 references] 
...
tan [1 reference] 
white [1 reference] 
yellow [12 references] 
---------- (* \textbf{Other moveable/interactable objects} *) ----------
alarm_clock [8 references] 
book [9 references] 
(*\color{gray} blinds [N/A references] ; The blinds on the windows*)
chair [2 references] 
cellphone [6 references] 
cd [6 references] 
credit_card [1 reference] 
curved_wooden_ramp [1 reference] 
desktop [4 references] 
doggie_bed [9 references] 
hexagonal_bin [12 references] 
key_chain [5 references] 
lamp [2 references] 
laptop [7 references] 
main_light_switch [2 references] ; The main light switch on the wall
mug [3 references] 
triangular_ramp [1 reference] 
(*\color{gray} green_triangular_ramp [N/A references] *)
pen [2 references] 
pencil [2 references] 
pillow [3 references] 
teddy_bear [2 references] 
watch [1 reference] 
---------- (* \textbf{Immoveable objects} *) ----------
bed [2 references] 
(*\color{gray} corner [N/A references] ; Any of the corners of the room*)
(*\color{gray} south_west_corner [N/A references] ; The corner of the room where the south and west walls meet*)
(*\color{gray} door [N/A references] ; The door out of the room*)
(*\color{gray} desk [N/A references] *)
(*\color{gray} desk_shelf [N/A references] ; The shelves under the desk*)
(*\color{gray} drawer [N/A references] ; Either drawer in the side table*)
top_drawer [2 references] ; The top of the two drawers in the nightstand near the bed.
floor [2 references] 
rug [2 references] 
(*\color{gray} shelf [N/A references] *)
(*\color{gray} bottom_shelf [N/A references] *)
(*\color{gray} top_shelf [N/A references] *)
side_table [2 references] ; The side table/nightstand next to the bed
(*\color{gray} sliding_door [N/A references] ; The sliding doors on the south wall (big windows)*)
(*\color{gray} east_sliding_door [N/A references] ; The eastern of the two sliding doors (the one closer to the desk)*)
wall [4 references] ; Any of the walls in the room
(*\color{gray} north_wall [N/A references] ; The wall with the door to the room*)
(*\color{gray} south_wall [N/A references] ; The wall with the sliding doors*)
(*\color{gray} west_wall [N/A references] ; The wall the bed is aligned to*)
---------- (* \textbf{Non-object-type predicate arguments} *) ----------
(*\color{gray} back [N/A references] *)
(*\color{gray} front [N/A references] *)
(*\color{gray} left [N/A references] *)
(*\color{gray} right [N/A references] *)
(*\color{gray} sideways [N/A references] *)
(*\color{gray} upright [N/A references] *)
(*\color{gray} upside_down [N/A references] *)
(*\color{gray} front_left_corner [N/A references] ; The front-left corner of a specific object (as determined by its front)*)
---------- (* \textbf{Undescribed types} *) ----------
(*\color{red} parseinfo [657 references]*)
(*\color{red} term [657 references]*)
\end{lstlisting}
